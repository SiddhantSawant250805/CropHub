    ...
    <div className="min-h-screen flex" style={{ background: "rgb(var(--surface))" }}>
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden items-center justify-center"
        ...
        <div className="relative z-10 max-w-sm px-12">
          ...
          <h1 className="text-4xl font-outfit font-black mb-4 leading-tight" style={{ color: "rgb(var(--on-surface))" }}>
            Grow with<br /><span style={{ color: "#00e87a" }}>Confidence.</span>
          </h1>
          <p className="mb-10 text-sm leading-relaxed" style={{ color: "rgba(186,203,186,0.7)" }}>
            ...
            {benefits.map((b) => (
              <li key={b} className="flex items-center gap-3">
                <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0"
                  ...
                  <CheckCircle2 className="w-3.5 h-3.5" style={{ color: "#00e87a" }} />
                </div>
                <span className="text-sm" style={{ color: "rgba(217,230,220,0.75)" }}>{b}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <h2 className="text-3xl font-outfit font-black mb-1" style={{ color: "rgb(var(--on-surface))" }}>Create account</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="name" className="text-sm font-medium" style={{ color: "rgba(217,230,220,0.8)" }}>Full Name</Label>
              <div className="relative">
                <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: "rgba(186,203,186,0.5)" }} />
            ...
            <motion.button type="submit" disabled={isSubmitting}
              ...
              className="btn-primary w-full h-12 rounded-xl font-semibold flex items-center justify-center gap-2 mt-2">
              {isSubmitting ? (
          ...
          <button disabled className="w-full h-12 rounded-xl flex items-center justify-center gap-2 text-sm font-medium opacity-50 cursor-not-allowed"
          ...
          <p className="text-center text-sm mt-8" style={{ color: "rgba(186,203,186,0.6)" }}>
            ...
            <Link to="/login" className="font-semibold hover:underline" style={{ color: "#00e87a" }}>Log in</Link>
          </p>
        </motion.div>
      </div>
    </div>
